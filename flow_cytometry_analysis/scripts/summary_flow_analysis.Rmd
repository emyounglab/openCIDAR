...
Make summary / lookup table of positive and negative numbers
...
```{r}
alpha = 0.05
pos_df_summary$CI_95_LB <- pos_df_summary$gmean - qnorm(1-alpha/2)*pos_df_summary$std_err
pos_df_summary$CI_95_UB <- pos_df_summary$gmean + qnorm(1-alpha/2)*pos_df_summary$std_err
```
...
```{r}
# Put in data
p <- ggplot(pos_df_summary, aes(x=fct_relevel(promoter, "J23103", "J23106", "J23100"), y=gmean, label = rbs, fill = growth_defect)) 
...
# Add formatting
p <- p + scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
              labels = trans_format("log10", math_format(10^.x))) + geom_point(size=2, shape=23) + facet_wrap(~fct_relevel(species,"e_coli","p_putida","c_necator","k_nataicola"), ncol=4, labeller = as_labeller(spec.labs)) + geom_text_repel(size=2) + ggtitle("Standardized measurements enable equivalent part lookup across species") + xlab("Promoter") + ylab(" Geometric mean MEFL") + guides(fill = guide_legend(title = "Growth defect", reverse = TRUE)) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + geom_errorbar(aes(ymin = CI_95_LB, ymax = CI_95_UB),width=0.25,size=0.25)

# Save and display plot
ggsave(filename="../outputs/gmean_LUT_w_error.pdf")
p
```
...
Need to fix code below - missing openCIDAR data
...
Combine promoter / RBS combination together into one variable
...
Question: do some of the strains have higher or lower expression than others, after subtracting autofluorescence and blocking by promoter/RBS combination?

Look at rank order plots
...
#p4 <- p3 + scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x))) + geom_point(size=2, shape=23) + ggtitle("Rank order comparison") + xlab("Promoter / RBS pair") + ylab("Arithmetic mean MEFL") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +facet_wrap(~fct_relevel(species,"e_coli","p_putida","c_necator","k_nataicola"),ncol = 4)
#ggsave(filename="../outputs/rank_order_comparison_facet.pdf")
#p4
...
p5
```
Now for geometric mean with just positive data
...
```{r}
p <- plot(pos_df_summary$gmean[pos_df_summary$species=="e_coli"],pos_df_summary$gmean[pos_df_summary$species=="p_putida"])
p
```
...
Using correlation table package
...
Can we build a linear model for each strain by design?
...
These above tables give us an estimate of a reference level based on promoter J23100 and rbs B0032m with adjustments up and down for each design. How do the models change when we separate out promoter and rbs? Note R^2 pretty low for Pp
...
The center changes some, but we can also use this as a way to tease out "effect size" for each promoter or RBS in terms of driving expression very high or low from the middle. Note that the R^2 actually decreased for Ec...

To get each factor teased out separately, can we set the empty vector as the reference level?
...
What happens if we exclude PJ23100 from the dataset
...
Need to figure out why coefficients aren't defined because of singularities - is it due to DoF? Interesting that adding in interatction terms changed R^2 so much with k_nataicola

Can we use this to automatically pick out promoter / RBS pairs that have strong interactions (i.e. aren't orthogonal parts) and to also compare how parts drive expression? How to visualize this data?


Can we also repeat this with species as a blocking variable?
